<?php
/*
Plugin Name: Custom Post SlideShow
Description: Custom Post slide show lets you create Custom Post Types and custom Taxonomies in a user friendly way.
Version: 1.1.5
Author: LuminoGuru +Team
Author URI: http://luminoguru.com/
Text Domain: custom-post-slideshow
Domain Path: /lang
...
class Cptm
{
	...
	function __construct()
	{
		...
		add_action( 'wp_enqueue_scripts', array($this, 'slick_styles') );
		add_action( 'add_meta_boxes', array($this, 'cptm_create_meta_boxes') );
		add_action( 'add_meta_boxes', array($this, 'cptm_slide_meta_boxes'),10,2 );
	...
	public function init()
	{
		...
		$labels = array(
			'name' => __( 'Custom Post slideshow', 'custom-post-type-maker' ),
	...
	public function cptm_admin_menu()
	{
		...
		add_menu_page( __('CPT Maker', 'custom-post-type-maker' ), __('Custom Post SlideShow', 'custom-post-type-maker' ), 'manage_options', 'edit.php?post_type=cptm', '', 'dashicons-layout' );
...
public function slick_styles()
	{
		wp_register_style( 'slick', $this->dir . '/css/slick.css' );
		wp_register_style( 'slic_theme', $this->dir . '/css/slick-theme.css' );
		
	    wp_enqueue_style( 'slick' );
	    wp_enqueue_style( 'slic_theme' );

	    wp_register_script( 'slick_js', $this->dir . '/js/slick.min.js', 'jquery', '1.9.0', true );
	...
	public function cptm_create_custom_post_types()
	{
		...
		if ( $cptm_post_types ) {
			foreach ( $cptm_post_types as $cptm ) {
				...
				$cptm_shortcode        = ( array_key_exists( 'cptm_shortcode', $cptm_meta ) && $cptm_meta['cptm_shortcode'][0] ? $cptm_meta['cptm_shortcode'][0] : '' );
				...
				if ( is_array( $cptms ) ) {
					...
					foreach ($cptms as $cptm_post_type) {
						...
						$args = array(
							'labels'              => $labels,
							'description'         => $cptm_post_type['cptm_shortcode'],
							'shortcode'           => $cptm_post_type['cptm_description'],
	...
	public function cptm_slide_meta_boxes($post)
	{
		...
		if( $cptm_singular_name ){
			$posts = array( $cptm_singular_name );
		}else{
			$posts = array( $cptm_label );
		}
		

		foreach ( $posts as $cfc ) {

			add_meta_box(
				'cptm_slider_options',
				__( 'Slider', 'custom-post-type-maker' ),
	...
	{	
		// nonce
		wp_nonce_field( 'slider_meta_box_callback_nonce_action', 'slider_meta_box_callback_nonce_field' );
	    ...
	    if ( isset( $gallery_data['image_url'] ) ) 
	    {
	        ...
	        <div class="field_row">
	        	<div class="remove_slide" onclick="remove_slide(this)">X</div>
	        	<div class="field_half"><h5> Slide <?php echo $i + 1;?></h5></div>
	        	<div class="field_half">
	        		<div class="form_field">
	        			<label>Slide Title</label>
	        			<input type="text" name="gallery[slide_title][]" value="<?php esc_html_e( $gallery_data['slide_title'][$i] );?>" >
	        		</div>
	        	</div>
	        	<div class="field_half">
	        		<div class="form_field">
	        			<label>Image credit</label>
	        			<input type="text" name="gallery[image_credit][]" value="<?php esc_html_e( $gallery_data['image_credit'][$i] );?>" >
	        		</div>
	        	</div>
				<div class="field_full tmp">
					<div class="form_field">
		        ...
		        <div class="field_full tmp">
		            ...
		            <textarea id="slide_intro"  name="gallery[slide_intro][]" ><?php esc_html_e( $gallery_data['slide_intro'][$i] ); ?></textarea>
	    ...
	    <div style="display:none" id="master-row">
		    <div class="field_row">
	    		...
	    		<div class="field_half">
	        		<div class="form_field">
	        			<label>Slide Title</label>
	        			<input type="text" name="gallery[slide_title][]" value="" >
	        		</div>
	        	</div>
	        	<div class="field_half">
	        		<div class="form_field">
	        			<label>Image credit</label>
	        			<input type="text" name="gallery[image_credit][]" value="" >
	        		</div>
	        	</div>
		        <div class="field_full tmp">
		            <div class="form_field">
		        ...
		        <div class="field_full tmp">
		            <div class="form_field">
		                ...
		                <?php 
		                $value= '';
		                wp_editor( $value, 'slide_intro', array(
						    'textarea_name' => 'gallery[slide_intro][]',
						    'textarea_rows' => 10,
						) ); ?>
		            <!--      <textarea id="slide_intro" class="meta_slide_intro" value="" type="text" name="gallery[slide_intro][]" ></textarea> -->   
		            </div>
		        </div>
		        <div class="clear"></div>
		    </div>
	    ...
	    <div id="add_field_row">
	 	...
	 	<style type="text/css">
			...
			#dynamic_form .field_btn {
				display: inline-block;
			}
			#dynamic_form label {
				padding:0 6px;
			}
			#dynamic_form .image_wrap {
				display: inline-block;
				margin: 0 10px;
			}
			#dynamic_form .field_half h5 {
				margin-bottom: 0;
			}
	...
	public function cptm_meta_box( $post ) {
		...
		$cptm_label                         = isset( $values['cptm_label'] ) ? esc_attr( $values['cptm_label'][0] ) : '';
		$cptm_singular_name                 = isset( $values['cptm_singular_name'] ) ? esc_attr( $values['cptm_singular_name'][0] ) : '';
		$cptm_shortcode                   = isset( $values['cptm_shortcode'] ) ? esc_attr( $values['cptm_shortcode'][0] ) : '';
		$cptm_description                   = isset( $values['cptm_description'] ) ? esc_attr( $values['cptm_description'][0] ) : '';

		//cptm_slide_meta_boxes($cptm_singular_name);
		// print_r($cptm_singular_name);
		// Custom post icon (uploaded)
		...
		$cptm_supports_author               = $pagenow === 'post-new.php' ? 'author' : $cptm_supports_author;
		$cptm_supports_featured_image       = $pagenow === 'post-new.php' ? 'thumbnail' : $cptm_supports_featured_image;
		?>
		<table class="cptm">
			<tr>
				<td class="label">
	...
	<!-- 	    <tr>
				<td class="label top">
					<label for="cptm_shortcode"><?php _e( 'Shortcode', 'custom-post-type-maker' ); ?></label>
					<p><?php _e( 'A short descriptive summary of what the post type is.', 'custom-post-type-maker' ); ?></p>
				</td>
				<td>
					<input type="text" readonly="readonly" name="cptm_shortcode" id="cptm_shortcode" class="widefat" tabindex="3" value="<?php echo $cptm_shortcode; ?>" />
				</td>
			</tr> -->
			<tr>
				<td class="label top">
			...
			<tr>
				...
				<td>
					...
					<input type="checkbox" tabindex="26" name="cptm_supports[]" id="cptm_supports_featured_image" value="thumbnail" <?php checked( $cptm_supports_featured_image, 'thumbnail' ); ?> /> <label for="cptm_supports_featured_image"><?php _e( 'Featured Image', 'custom-post-type-maker' ); ?> <span class="default">(<?php _e( 'default', 'custom-post-type-maker' ); ?>)</span></label><br />
					<input type="checkbox" tabindex="27" name="cptm_supports[]" id="cptm_supports_author" value="author" <?php checked( $cptm_supports_author, 'author' ); ?> /> <label for="cptm_supports_author"><?php _e( 'Author', 'custom-post-type-maker' ); ?> <span class="default">(<?php _e( 'default', 'custom-post-type-maker' ); ?>)</span></label><br />
	...
	public function cptm_save_post( $post_id )
	{
	    ...
	    if ( isset( $_POST['gallery'] )) 
	    {
	        ...
	        for ($i = 0; $i < count( $_POST['gallery']['slide_title'] ); $i++ ) 
	        {
	            if ( '' != $_POST['gallery']['slide_title'][ $i ] ) 
	            {
	                $gallery_data['slide_title'][]  = $_POST['gallery']['slide_title'][ $i ];
	            }
	        }
	         for ($i = 0; $i < count( $_POST['gallery']['image_credit'] ); $i++ ) 
	        {
	            if ( '' != $_POST['gallery']['image_credit'][ $i ] ) 
	            {
	                $gallery_data['image_credit'][]  = $_POST['gallery']['image_credit'][ $i ];
	            }
	        }
	        
	        for ($i = 0; $i < count( $_POST['gallery']['slide_intro'] ); $i++ ) 
	        {
	            if ( '' != $_POST['gallery']['slide_intro'][ $i ] ) 
	            {
	                $gallery_data['slide_intro'][]  = $_POST['gallery']['slide_intro'][ $i ];
	            }
	        }
		...
		if ( isset($_POST['cptm_shortcode']) )
			update_post_meta( $post_id, 'cptm_shortcode', esc_textarea( $_POST['cptm_shortcode'] ) );
	...
	function cptm_custom_columns( $column, $post_id )
	{
		switch ( $column ) {
			...
			case "description":
				...
				break;
			case "shortcode":
				echo get_post_meta( $post_id, 'cptm_shortcode', true);
				break;
		}
	}
	...
	function cptm_admin_footer()
	{
		...
		if ( 'cptm' == $post_type ) {
			...
			// Get all Custom Post Types created by Custom Post slideshow
			$cptm_posts = get_posts( array( 'post_type' => 'cptm' ) );
			// Remove all Custom Post Types created by the Custom Post slideshow plugin
			...
			<div id="cptm-cpt-overview" class="hidden">
				...
				<p><?php _e( 'The Custom Post Types below are registered in WordPress but were not created by the Custom Post slideshow plugin.', 'custom-post-type-maker' ); ?></p>
					...
					<thead>
						<tr>
							...
							<th scope="col" id="title" class="manage-column column-title">
								<span><?php _e( 'Post Title', 'custom-post-type-maker' ); ?></span><span class="sorting-indicator"></span>
							</th>
					...
					<tfoot>
						<tr>
							...
							<th scope="col" class="manage-column column-title">
								<span><?php _e( 'Custom Post Title', 'custom-post-type-maker' ); ?></span><span class="sorting-indicator"></span>
							</th>
		...
		if ( 'cptm_tax' == $post_type ) {
			...
			// Get all custom Taxonomies created by Custom Post slideshow
			$cptm_tax_posts = get_posts( array( 'post_type' => 'cptm_tax' ) );
			// Remove all custom Taxonomies created by the Custom Post slideshow plugin
			...
			<div id="cptm-cpt-overview" class="hidden">
				...
				<p><?php _e( 'The custom Taxonomies below are registered in WordPress but were not created by the Custom Post slideshow plugin.', 'custom-post-type-maker' ); ?></p>
				<table class="wp-list-table widefat fixed posts" cellspacing="0">
					<thead>
						<tr>
							<th scope="col" id="cb" class="manage-column column-cb check-column">
